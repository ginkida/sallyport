...
| Arbitrary JS | Per-domain `allowEvaluate` opt-in; fixed-literal probes (`fetch_in_page` body, keystroke password probe, `snapshot`'s DOM-fallback walker, `screenshot`'s `document.visibilityState` probe) interpolate no agent input and need only the allowlist | `extension/src/tools/gates.ts:ensureEvaluateAllowed`; `fetch.ts`, `focus.ts`, `domtree.ts`, `screenshot.ts` |
