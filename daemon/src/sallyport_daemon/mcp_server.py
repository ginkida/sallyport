...
TOOLS: list[Tool] = [
    ...
    Tool(
        ...
        description=(
            ...
            "coordinates); it is intersected with the viewport. BOTH are "
            "OBSERVABLE ON THE PAGE: Chrome implements a clipped capture by "
            "resizing the tab's view to the clip and restoring it afterwards, so "
            "the page reflows and fires `resize`. Prefer a plain full-viewport "
            "capture unless you actually need the crop or the smaller payload — "
            "on a layout-sensitive page the resize can change what you capture. In broker mode "
